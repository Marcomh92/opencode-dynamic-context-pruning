...
| `prune.messages.nextBlockId`                            | `:67`                                    | **Implementation note (BUG-089 review):** `mergeInheritedBlocks` (`lib/compress/state.ts:327`) sets `nextBlockId = parentMaxBlockId + 1` when `parentMaxBlockId >= state.nextBlockId`. Plain `max(parent.nextBlockId, state.nextBlockId)` is WRONG — it collides with the parent's last blockId. The `+1` is required because `allocateBlockId` (`lib/compress/state.ts:8-17`) returns THEN increments, so `nextBlockId` is NEXT-FREE, not LAST-USED. Matches canonical `loadPruneMessagesState` pattern (`lib/state/utils.ts:366-371`). |
...
| `lastCompaction`                                        | —                                        | **DELETED (architect flag #7):** Not persisted (see STATE*PERSISTENCE.md persisted-vs-in-memory table). Recomputed from B's own messages at `lib/state/state.ts:221-223` before the hook point. The recomputed value is \_more* correct than A's. Nothing to copy.                                                                                                                                                                                                                                                                       |
...
14. **BUG-031 fully superseded at v4** — `recoveryForced`/`nonCompactingRunCount`/`recoveryFadeCounter` round-trip on every `saveSessionState`/`loadSessionState` call, not just on fork. See `lib/state/state.ts:281-301` (loader) and `lib/state/persistence.ts:142-144` (saver). Stale comment block at `state.ts:269-271` removed in BUG-089 review pass.
