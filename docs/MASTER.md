...
| Session fork (OpenCode) | A UI-forked OpenCode session; unrelated to the repo fork / fork protocol. Detected by `detectParentSessionFromTitle` in `lib/state/utils.ts`. See `docs/DECISIONS/002-compression-state-is-session-scoped.md`. |
