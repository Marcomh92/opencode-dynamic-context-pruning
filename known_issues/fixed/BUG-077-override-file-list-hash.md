...
Added `logger.info` drift-detection line at `lib/prompts/store.ts:441` (and `:458` for README); byte comparison already detects drift.
...
- **Bonus**: byte comparison does NOT cover user's _override_ files — only the bundled defaults. Override drift is the user's choice and should not be logged. The `existing === managedContent` comparison is byte-equal, not whitespace-normalized — if the bundled source has trailing-whitespace drift (cf. BUG-072), every reload rewrites the file.
