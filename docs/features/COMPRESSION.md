...
| INV-11 | Block-placeholder regex: `/\(b(\d+)\)                                                                                                                                                                                                                                                                                                                                         | \{block\_(\d+)\}/gi`. Validation keeps only placeholders whose block is active and in the selection's `requiredBlockIds`.                                                                                                        | `lib/compress/range-utils.ts:13, 233-274` |
...
| INV-21 | `protectUserMessages: true` protects only the last N real user messages (`N = compress.protectUserMessagesCount`, default 1, clamped `>= 1`). Synthetic / ignored user messages do not count toward N. Range-mode only, messages fully consumed by `stripPatterns` also do not count toward N (stripPatterns is not consulted in message mode — see `docs/CONFIGURATION.md`). | `lib/messages/query.ts:111-137`, `lib/compress/protected-content.ts:13-68`, `lib/messages/priority.ts:37-44`, `lib/messages/inject/inject.ts:180-192`, `lib/compress/message-utils.ts:165, 244`, `lib/compress/range.ts:134-146` |
