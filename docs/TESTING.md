...
| Concern | Test file(s) |
|---|---|
| Hooks | `hooks-permission.test.ts`, `system-prompt-handler.test.ts` |
| Compress | `compress-message.test.ts`, `compress-range.test.ts`, `compress-range-placeholders.test.ts`, `compress-protocol.test.ts`, `compression-groups.test.ts`, `compression-targets.test.ts`, `validator-wiring.test.ts`, `decompress-prune-tools-cleanup.test.ts` |
| Token math | `token-counting.test.ts`, `token-usage.test.ts` |
| Messages | `message-utils.test.ts`, `message-ids.test.ts`, `message-priority.test.ts`, `append-idempotency.test.ts` |
| Config | `config-merge-protected-tools.test.ts` |
| State | `state-schema-version.test.ts`, `state-max-age.test.ts` |
| Persistence | `coalesce-save-session.test.ts`, `savecontext-rate-limit.test.ts`, `stats-race.test.ts` |
| Permissions | `host-permissions.test.ts`, `hooks-permission.test.ts` |
| UI | `notification-header.test.ts`, `desktop-notifications.test.ts`, `prompts.test.ts` |
| Synthetic (fork-protocol) | `synthetic-compress-burn.test.ts`, `synthetic-user-message-stability.test.ts`, `subagent-cache.test.ts` |
| Misc | `update.test.ts`, `protected-patterns.test.ts` |
