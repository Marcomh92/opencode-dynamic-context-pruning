...
| `stats.inheritedPruneTokens`                                                                 | `parentState.stats.totalPruneTokens` (parent's own savings) + `parentState.stats.inheritedPruneTokens` (multi-gen accumulation) | Set at fork time; multi-gen accumulates. `loadAllSessionStats` sums `totalPruneTokens` only (correct by construction). BUG-088 fixed. |
...
| 4a   | `lib/state/inherit.ts:365-398`                          | Scan storage dir; `stateRetentionDays` mtime pre-filter skips files older than the retention window before `readFile` + parse (BUG-092). One summary log line replaces per-file drops. |
...
| 9    | `lib/state/inherit.ts:416-445`                          | Copy `prune.tools`, recovery flags, `stats.inheritedPruneTokens` (parent's total + parent's inherited — see "What gets inherited"); re-derive `manualMode`.                            |
...
The candidate scan's mtime pre-filter (BUG-092, `lib/state/inherit.ts:365-398`) skips files older than `compress.stateRetentionDays` with one `fs.stat` syscall, avoiding the more expensive `readFile` + `JSON.parse` + schema gate for stale entries. One summary log line replaces the per-file debug drop lines that previously inflated logs on machines with accumulated test fixtures.

The sweep on save (BUG-092, `lib/state/persistence.ts:101-155`, called from `saveSessionState`) deletes DCP state files older than `compress.stateRetentionDays` once per plugin-process run (module-level `sweepDone` throttle). Best-effort per-file try/catch — a stuck file or permission race never aborts the sweep. `null` disables it. Keeps the storage dir bounded; complements the load-time `stateMaxAgeDays` gate.
